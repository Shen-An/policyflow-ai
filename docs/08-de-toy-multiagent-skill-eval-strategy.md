...
| 真实语料 parity（词面臂已跑通·embedding 臂受限于 chat 端点） | **词面臂已完成（诚实）** | 2026-09-24 | `scripts/graph_realcorpus_parity.py` 参数化出 `bm25_only` 词面臂（BM25 直读 `content_text`，**不需 provider**）。真实 CRUD `questanswer_1doc` 50 例 + 200 干扰进 `eval_test`，`ROUTE_VIA_GRAPH_ADAPTER=False vs True` 逐例结论（有序 doc id + Hit@K/MRR + passed + score）**逐字相等** → `artifacts/graph/stage3/realcorpus_parity_bm25.json`（`PARITY_OK`，50 例，`route_eval` 遥测=1）。首个**真实语料级**切换透明性证据。**边界**：① 只跑词面臂；embedding/hybrid 臂需真实 chat+embedding 双 provider（in-process LightRAG 索引期用 chat LLM 抽取实体/关系）。harness 已升级为**分离双 provider**（`EVAL_CHAT_*`/`EVAL_EMBED_*` 六个环境变量注密文密钥、embedding 维度自探测、索引完成硬门控——`indexed==0` 拒绝空索引 parity，绝不伪造绿）。已实测：embedding 端点（NVIDIA `nvidia/llama-nemotron-embed-vl-1b-v2`，dim=2048）连通 200；**但** chat 端点（`token.sensenova.cn` `deepseek-v4-flash`）**两处受限**——(a) 配额耗尽（`insufficient_quota` / `inference exceeds tpm/rpm limit` 429），(b) 系推理模型，抽取长 prompt 下 `content` 空（正文进 `reasoning_content`，非有效抽取输出）→ LightRAG 逐 chunk `LLM response content is empty`，全部 doc `failed`。故 embedding 臂 parity **仍未产出**（非代码缺陷，是外部 chat 端点能力/配额限制）；换一个能承载抽取调用量、返回非空 `content` 的 chat 端点即可产出 `realcorpus_parity.json`。② 1-doc 整篇匹配 MRR/Hit 天然偏高，此处只主张 parity 不主张检索质量。不比生成答案文本（LLM 采样与 flag 独立） |
