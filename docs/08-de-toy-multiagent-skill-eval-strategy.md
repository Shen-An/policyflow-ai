...
| 真实语料 parity（词面臂已跑通·embedding 臂待端点） | **词面臂已完成（诚实）** | 2026-09-24 | `scripts/graph_realcorpus_parity.py` 参数化出 `bm25_only` 词面臂（BM25 直读 `content_text`，**不需 provider**）。真实 CRUD `questanswer_1doc` 50 例 + 200 干扰进 `eval_test`，`ROUTE_VIA_GRAPH_ADAPTER=False vs True` 逐例结论（有序 doc id + Hit@K/MRR + passed + score）**逐字相等** → `artifacts/graph/stage3/realcorpus_parity_bm25.json`（`PARITY_OK`，50 例，`route_eval` 遥测=1）。首个**真实语料级**切换透明性证据。**边界**：① 只跑词面臂；embedding/hybrid 臂因用户端点（`token.sensenova.cn/v1`）**仅 chat 无 embedding 模型**未执行，harness 对该臂仍硬门控 `PROVIDER_NOT_CONFIGURED` 退 2（绝不伪造绿），补跑只需一个 embedding 端点：`... <n> <d> hybrid_lightrag_bm25`；adapter 与检索策略无关的纯透传，词面臂已覆盖同一切换代码路径。② 1-doc 整篇匹配 MRR/Hit 天然偏高，此处只主张 parity 不主张检索质量。不比生成答案文本（LLM 采样与 flag 独立） |
