...
| 12 | 发邮件超时后能不能直接回滚重发 | 外部副作用的幂等、状态核对与补偿 |
...
## Q11. 场景：模型重试、Tool 循环、低质 RAG 同时发生

**场景**
面试官问：「LLM 遇到 429 在重试，模型又连续调用检索 Tool，检索结果质量还不好，最后 Critique 又要求改稿。你怎么保证它不会一直调用、越来越慢，最后还输出错误答案？」

### 当前项目已经实现什么

- LLM provider 已有最大尝试次数和退避重试。
- Answer Tool loop 已有最大轮数，默认 3 轮。
- Reflection 已有最大轮数，默认 2 轮。
- 无证据默认 hard refuse。
- 已有引用、数字、claim–evidence 词重叠检查。
- 请求级 Turn Budget 统一限制 LLM、检索、Tool 次数和整轮耗时。
- Retrieval Quality Gate 会识别空结果、偏题和 query rewrite 漂移，最多回退原问题再检索一次。
- Compliance 输出 `PASS/REVISE/REFUSE`；`REVISE` 只定向改稿一次并复检，仍失败则安全拒答。

### 原来存在的问题

原来的模块限制是**各管各的**：

- LLM 重试次数没有和 Tool、检索、Critique 共用一个总预算。
- 检索“返回了文档”不代表文档真的可靠，相似词或错误制度可能被放行。
- 缺引用、引用错配等问题目前可能只是 warning，不一定阻止答案发布。
- Critique 自己失败时，不能保证问题草稿一定被拦住。

所以最坏情况不是某一个环节无限循环，而是多个环节分别没有超限、合起来却调用很多次。

### 项目如何解决

1. **设置单轮总预算**
   不只限制 Tool 3 轮、Reflection 2 轮，还限制整轮最多多少次 LLM 调用、最多几次检索、最多耗时多久。任何环节重试都消耗同一个预算。

2. **对错误分类处理**
   - 429、网络超时、部分 5xx：有限重试。
   - 参数错误、权限错误：不重试，直接失败。
   - Memory、可选 Reflection 失败：记录告警并降级。
   - 无证据、引用硬错误：不能降级成猜测，必须拒答。

3. **增加检索质量门**
   检索后先判断资料是否与原问题相关，而不是只看有没有 Top-K。若 query rewrite 漂移或结果偏题，最多使用原问题再检索一次；仍不可靠就 hard refuse。

4. **增加答案发布门**
   将检查结果分成：
   - `PASS`：允许发布。
   - `REVISE`：按具体问题改一次，改完必须重新检查。
   - `REFUSE`：不能发布原稿，返回安全拒答或只保留有证据的部分。

5. **保证到上限后有确定出口**
   达到调用预算后不再继续 Tool 或 Critique。已经有可靠证据就生成简化答案；没有可靠证据就拒答，不能用“再试一轮”拖延退出。

### 面试回答模板

> 我会做三层限制：单节点最大轮数、单次调用超时、单轮请求总预算。检索结果还要先过质量门，答案生成后再过发布门。可恢复错误有限重试，证据问题直接拒答。这样不是要求模型自己决定什么时候停，而是由编排器硬性控制出口。

### 诚实边界

当前落地的是单体应用内的基础兜底：检索质量使用轻量确定性信号，不宣称能识别所有语义错配；达到预算时会终止链路并返回稳定错误/拒答，不宣称第三方服务一定可恢复。

### 可能追问

| 追问 | 短答 |
|---|---|
| 为什么不多重试几次提高成功率？ | 重试会增加延迟、成本和重复副作用风险；只对瞬时且幂等的错误有限重试 |
| 达到预算后一定拒答吗？ | 不一定；有可靠证据可返回简化答案，无可靠证据才 hard refuse |
| 是否用 LLM judge 判断答案正确？ | 可辅助高风险语义检查，但引用、数字、权限和空证据优先走确定性规则 |

---

## Q12. 场景：发邮件超时后能不能直接回滚重发

**场景**
系统调用邮件、日历或飞书 Tool。请求在 20 秒后超时，面试官问：「那就数据库 rollback，然后重新发送一次，不就行了吗？」
...
```text
PolicyFlow 发出“发送飞书消息”请求
  → 飞书服务器收到请求并成功发送消息
  → 飞书返回成功响应
  → 响应在网络中丢失
  → PolicyFlow 只看到了“请求超时”
```

这时 PolicyFlow 不知道消息到底有没有发出去，所以状态不是“失败”，而是**状态未知（unknown）**。
...
- 日历事件已经创建：调用“取消事件”。
- 邮件只是生成草稿：调用“删除草稿”。
- 飞书消息已经发送且平台支持撤回：调用“撤回消息”。
- 平台不支持撤回：记录执行结果并提示人工处理，而不是假装已经回滚。

### 项目如何解决

1. **给 Tool 分类**
   - `read`：查询类，可以有限重试。
   - `local_write`：只写本地数据库，可以使用事务 rollback。
   - `external_write`：邮件、日历、飞书等外部副作用，默认不能盲目重试。

2. **为外部操作生成幂等键**
   同一次业务操作始终携带相同的 `idempotency_key`。如果外部系统支持幂等，它收到重复请求时返回第一次的结果，而不是执行第二次。

3. **记录更准确的执行状态**
   - `pending`：正在执行。
   - `succeeded`：明确成功。
   - `failed`：明确失败。
   - `unknown`：超时或断网，不确定是否执行。
   - `compensated`：原操作成功，但后来执行了补偿动作。

4. **超时后先核对状态**
   如果外部平台支持查询，就用操作 ID 或幂等键查询是否已经执行；确认未执行后才允许重试。无法查询时，保持 `unknown` 并提示用户确认，不能自动重复发送。

5. **高风险操作执行前确认**
   真正发送邮件、创建日历、对外通知等动作，在执行前要求用户确认；查询、生成草稿等低风险动作可以自动执行。

6. **支持补偿，但不夸大成全局回滚**
   有反向 Tool 时自动或人工触发补偿；没有反向能力时保留审计记录并人工处理。

### 当前项目做到哪

- 已有 Tool 调用日志、白名单、真实 handler 超时、请求级 Tool 次数预算和 mock 企业连接器。
- Chat Tool 会生成稳定幂等键；相同操作明确成功时复用原结果，超时则记录 `unknown`，相同键不允许直接重发。
- 本地 Chat 持久化提交失败会执行数据库 rollback。
- Mock 企业连接器会明确标注 `status=mock`。
- 尚未实现真实外部平台的状态查询适配器、补偿 Tool 和高风险确认流程，因此不能宣称完整 Saga。

### 面试回答模板

> 数据库事务只能回滚本地未提交数据，不能撤销已经发生的外部副作用。外部调用超时后我不会立即重发，而是标记为 unknown，用幂等键或操作 ID 查询执行状态；确认未执行才重试。已经执行则返回原结果，必要时调用取消、删除或撤回等补偿操作。平台不支持补偿时就明确提示人工处理，不假装全局回滚。

### 可能追问

| 追问 | 短答 |
|---|---|
| 幂等键是不是一定有效？ | 需要外部平台或适配器支持；不支持时必须依靠状态查询和人工确认 |
| 生成邮件草稿也这么严格吗？ | 风险较低，可自动执行；真正发送属于 external_write，应更严格 |
| 为什么区分 failed 和 unknown？ | failed 表示确定没成功，可以考虑重试；unknown 表示可能已成功，直接重试可能重复执行 |
| 这是 Saga 吗？ | 当前只是单体 MVP 的幂等、状态核对和补偿意识，不宣称生产级分布式 Saga |
