...
export function AuditPage() {
  ...
  return (
    <section>
      ...
      {query.isPending ? <div role="status" className="mt-6 p-6">正在加载审计日志…</div> : query.isError ? (
        <div role="alert" className="mt-6 rounded-xl bg-red-50 p-5"><h3 className="font-semibold">审计日志加载失败</h3><p className="mt-1 text-sm">{query.error.message}</p><Button className="mt-3" onClick={() => void query.refetch()}>重新加载</Button></div>
      ) : query.data.items.length === 0 ? <div className="mt-6 grid min-h-48 place-items-center rounded-xl border border-dashed border-[var(--color-border)] bg-white"><p>没有符合条件的审计记录</p></div> : (
        <>
