...
- [~] T051 [US3] 在 `backend/app/api/routes_chat.py` 中将普通与 SSE Chat legacy endpoints 改为调用 graph service 的兼容 adapter，并记录 adapter 使用遥测和 Stage 9 删除条件（依赖 T049）
  ...
  - **2026-09-23 第六轮（可回退路由切换）：新增 `backend/app/graph/route_adapter.py::GraphRouteAdapter`；`routes_chat.post_chat`/`post_chat_stream` 现按 `request.app.state.settings.ROUTE_VIA_GRAPH_ADAPTER`（默认 `False`）分流：**开=经 `GraphRouteAdapter.chat`/`chat_events` 记录 removal-ledger 遥测（`route_chat`/`route_chat_stream`）并委派共享 pipeline 图；关=字节等价旧路径**。契约保持绿：`tests/integration/test_graph_route_adapter_live.py`（flag-ON `/api/chat` 富 `ChatResponse` 形状不变 + `usage_count("route_chat")>=1` + 遥测不含 payload；flag-OFF 无遥测）+ `test_graph_route_adapter.py`（4）+ f4/f5/f6（10）全绿。**仍保持 `[~]`**：底层执行的是 pipeline 图（Option A），**非** durable `GraphService`；「Chat/Eval 结论一致率 100%」Independent Test 需运行栈 + 真实语料，本机缺 LightRAG/Milvus 检索栈**未验证**。**
...
- [~] T053 [US3] 在 `backend/app/agents/pipeline.py` 中把 `AgentPipeline` 收敛为 graph service facade，删除第二套业务 stage 执行并保留限期兼容遥测（依赖 T049）
  ...
  - **2026-09-23 用户裁定「保持门控（推荐）」（ratified-deferred）：T053 的删除动作**明确留作 Stage 9 未来动作**，不在本阶段执行。理由：`ROUTE_VIA_GRAPH_ADAPTER` 默认 OFF，旧直连路径仍是当前生产默认在用路径；须先在真实部署把 flag 打开、在观察窗口内由 `AdapterUsageTelemetry.zero_use_over_window()` 证明旧路径零使用，方可安全删除。现在删除会移除在用默认路径并破坏前端契约。故 T053 保持 `[~]`，这是**经用户确认的有意推迟**，非遗漏。**
...
- [~] T054 [US3] 运行 `tests/contract/test_graph_state.py`、`tests/integration/test_graph_entrypoint_parity.py`、`tests/eval/test_evidence_gate_parity.py` 并将 parity 明细保存到 `artifacts/graph/stage3/`（依赖 T037–T053）
  ...
  - **2026-09-23 第七轮（真实语料 parity 的可运行入口，凭据缺口外部化）：新增 `scripts/graph_realcorpus_parity.py`——真栈 arm。硬门控：`LLM_BASE_URL`/models 为占位符或 `LLM_API_KEY_ENV` 空时打印 `PROVIDER_NOT_CONFIGURED` 退 2、**不写任何证据**（今日本机即此结果，已验证；绝不伪造绿）。配好真实 OpenAI 兼容 embedding/LLM 后，脚本经真实 HTTP 路由驱动生产栈：导入随机 CRUD `questanswer_1doc` 样本 + 干扰文档进 `eval_test` 测试库 → 用 `eval_types=["retrieval"]` **确定性检索评测**跑两遍（flag OFF 直连 `execute_eval_run` / flag ON 经 `GraphRouteAdapter.run_eval`）→ 逐例比较有序检索 doc id + Hit@K/MRR/passed/score，写 `artifacts/graph/stage3/realcorpus_parity.json`。刻意不比生成答案文本（LLM 采样与 flag 独立，会造成假 parity 失败）。**运行栈 + 凭据到位前，此项为外部依赖阻塞，未验证；不因此宣布 Checkpoint 达成。****

> **Phase 3 Checkpoint 诚实状态（2026-09-23）**：可验证部分已达成——所有生产入口（chat/stream/eval）共享单一图、经可回退 flag 切换、证据门控 parity 在确定性替身层 100%、durable PG checkpoint 重启可恢复（T050）、无未批准副作用（eval deterministic + 禁副作用）。**尚未达成（硬阻塞，非跳过）**：① 真实语料 100% 结论 parity——本机无真实 OpenAI 兼容 embedding/LLM provider（`.env` 为占位符、`LLM_API_KEY_ENV` 未设），亦无本地 LightRAG/Ollama/sentence-transformers 栈；**可运行入口已就绪**（`scripts/graph_realcorpus_parity.py`，今日本机验证为干净 `PROVIDER_NOT_CONFIGURED` 退 2、不写伪证据），待用户补齐凭据即可执行；② T053 删除第二套 stage 执行——**用户已裁定「保持门控」，作为 Stage 9 有意推迟**：须先在真实部署将 flag 默认打开、观察窗口内 `AdapterUsageTelemetry.zero_use_over_window()` 证明旧路径零使用，方可安全删除（否则删除的是当前默认在用路径，破坏契约）。二者均不可在本环境诚实完成，故 Checkpoint 暂不宣布达成。
