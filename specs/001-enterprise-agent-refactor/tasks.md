...
> **Phase 3 Checkpoint 诚实状态（2026-09-24，第九轮更新）**：Checkpoint 的可验证判据已达成——所有生产入口（chat/stream/eval）**默认经共享图 route adapter**（`ROUTE_VIA_GRAPH_ADAPTER` 默认 `True`，legacy 直连保留为可回退分支）、证据门控 / 结论 parity 现有**两层证据**：(a) 确定性替身层 flag-on==flag-off 逐字相等（`entrypoint_conclusion_parity.json`），(b) **真实 CRUD 语料**词面臂 `bm25_only` 50 例 off==on 逐字相等（`realcorpus_parity_bm25.json`，`PARITY_OK`）；检索证据门已合流到规范 `backend/app/retrieval/evidence_gate.py`（T048）、durable PG checkpoint 重启可恢复（T050）、无未批准副作用（eval deterministic + 禁副作用）。已完成 `[X]`：T037–T052、T054（本轮）。**诚实残留（非伪造绿）**：① embedding/hybrid 臂的真实语料 parity 未单独执行——harness 已升级为分离双 provider 并实测（第十轮）：embedding 端点（NVIDIA，dim=2048）连通 ✓，但 in-process LightRAG 索引期需 chat LLM 做实体抽取，而用户 chat 端点（SenseNova `deepseek-v4-flash`）配额耗尽（429 `insufficient_quota`）+ 推理模型抽取 `content` 空，导致 doc 全 `failed`、harness 正确拒绝空索引 parity。属外部 chat 端点能力/配额限制而非代码缺陷；因 adapter 是与检索策略无关的纯透传、词面臂已跑通同一切换路径，此臂属边际验证，换一个能承载抽取调用量且返回非空 `content` 的 chat 端点即可补跑（`EVAL_CHAT_*`/`EVAL_EMBED_*` 注密钥 → `scripts/graph_realcorpus_parity.py <n> <d> hybrid_lightrag_bm25`）；② T053 删除第二套 stage 执行——须先在**真实部署观察窗口**内由 `AdapterUsageTelemetry.zero_use_over_window()` 证明 legacy 回滚分支零使用，且需先重建 durable graph 富输出面（~1200 行）；此窗口本会话无法流逝，**Stage 9（T159）有意推迟**，不属 Checkpoint 硬判据。**结论**：Phase 3 US3 已可独立演示且切换透明性在真实语料上得证；除上述两项诚实残留外无缺口。是否据此正式宣布 Checkpoint「达成」交由用户裁定——本会话不擅自宣布，也绝不为此伪造 embedding 臂或 T053 的绿。
